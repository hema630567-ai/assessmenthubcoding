...
public class CodeExecutionService {
    ...
    public ExecutionResult submitCode(String code, Problem problem, String language) {
        ...
        boolean noTestCases = inputs.stream()
            .allMatch(x -> x == null || x.trim().isEmpty());

        if (noTestCases &&
            problem.getSampleInput() != null &&
            problem.getSampleOutput() != null) {

            inputs = new ArrayList<>();
            outputs = new ArrayList<>();

            inputs.add(problem.getSampleInput());
            outputs.add(problem.getSampleOutput());
        }
