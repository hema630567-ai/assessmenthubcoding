...
function CodeEditor() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/submissions/all`, getAuthHeader());
  ...
  const handleAction = async (isSubmit) => {
    ...
    try {
      if (isSubmit) {
        ...
        const res = await axios.post(`http://localhost:8080/api/submissions/submit`, {
          code: userCode, 
          language: language,
        ...
        const res = await axios.post(
          `http://localhost:8080/api/submissions/run?langId=${langIdMap[language]}`, 
  ...
  return (
    <div style={{background: '#0a0a0a', color: '#e2e8f0', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: 'Segoe UI, sans-serif'}}>
      <header style={{height: '55px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 25px', background: '#111', borderBottom: '1px solid #333'}}>
        <div style={{fontWeight: '900', fontSize: '1.5rem', color: '#fff'}}>Quiz<span style={{color: '#3b82f6'}}>App</span></div>
